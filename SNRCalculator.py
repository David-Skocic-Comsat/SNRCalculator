...
def plot_frequency_response(frequencies, power_spectrum_linear, signal_mask, noise_floor_dB, threshold_dB, title="Power Spectral Density"):
    ...
    plt.axhline(noise_floor_dB, color='green', linestyle='--', label='Noise Floor')
    plt.axhline(signal_peak_dB, color='red', linestyle='--', label='Peak Power')
    plt.axhline(threshold_dB, color='yellow', linestyle='--', label='Threshold')

    if bandwidth > 0:
        plt.axvline(bw_low, color='purple', linestyle='--', label=f'Start: {bw_low:.0f} Hz')
        plt.axvline(bw_high, color='purple', linestyle='--', label=f'End: {bw_high:.0f} Hz')
...
def compute_noise_floor_and_threshold_dB(power_spectrum_linear, threshold_multiplier):
    ...
    signal_peak_dB = np.max(power_spectrum_dB)
    threshold_dB = noise_floor_dB + (signal_peak_dB - noise_floor_dB) * threshold_multiplier
...
def get_power_spectrum(iq_data, sample_rate):
    frequencies, power_spectrum_linear = signal.welch(iq_data, fs=sample_rate, nperseg=1024)
...
def compute_signal_mask(power_spectrum_dB, threshold_dB):
    ...
    start_of_signal = np.where(power_before_peak_dB[::-1] <= threshold_dB)[0][0]
    end_of_signal = np.where(power_after_peak_dB <= threshold_dB)[0][0]

    signal_mask = (np.arange(len(power_spectrum_dB)) >= signal_peak_index-start_of_signal) & (np.arange(len(power_spectrum_dB)) < signal_peak_index+end_of_signal)
...
def main():
    ...
    frequencies, power_spectrum_linear, power_spectrum_dB = get_power_spectrum(iq_data, sample_rate)
    ...
    noise_floor_dB, threshold_dB = compute_noise_floor_and_threshold_dB(power_spectrum_linear, threshold_multiplier=.15)
    signal_mask = compute_signal_mask(power_spectrum_dB,threshold_dB)

    plot_frequency_response(frequencies, power_spectrum_linear, signal_mask, noise_floor_dB, threshold_dB, title=f"PSD of {xml_path.name}")
